...
public class CollisionAvoidance {
  ...
  private static Pose2d goalPose = new Pose2d();
    ...
    public static Optional<SuperstructurePosition> plan(
        ...
        Pose2d xInterceptionPoint = new Pose2d(x1,(goalPoseY - currentPoseY)/(goalPoseX - currentPoseX) * (x1 - currentPoseX) + currentPoseY,Rotation2d.fromDegrees(0));
        Pose2d yInterceptionPoint = new Pose2d(y1,(y1-currentPoseY+currentPoseX)/((goalPoseY-currentPoseY)/(goalPoseX-currentPoseX)),Rotation2d.fromDegrees(0));
        Pose2d y2InterceptionPoint = new Pose2d(y2,(y2-currentPoseY+currentPoseX)/((goalPoseY-currentPoseY)/(goalPoseX-currentPoseX)),Rotation2d.fromDegrees(0));
  ...
  private static SuperstructurePosition poseToSuperstructurePosition(Pose2d pose){
    return new SuperstructurePosition(pose.getY()-(Math.sin(Math.cos(pose.getX()/wristLength))*wristLength), Math.cos(pose.getX()/wristLength));
  }
  private static double distancefromPoses(Pose2d currentPose, Pose2d goalPose){
    return Math.sqrt((currentPose.getX()-goalPose.getX())+(currentPose.getY()-goalPose.getY()));
