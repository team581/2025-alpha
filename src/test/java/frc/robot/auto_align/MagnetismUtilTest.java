...
public class MagnetismUtilTest {
  ...
  void testMagnetizedMagnitudeSameAsInputMagnitude(){
    ...
    ChassisSpeeds magnetizedSpeed = MagnetismUtil.getMagnetizedChassisSpeeds(robotSpeed, robotPose, goalPose);
    Assertions.assertEquals(getMagnitudeFromChassisSpeeds(robotSpeed), getMagnitudeFromChassisSpeeds(magnetizedSpeed));
  }
  ...
  void testMagnetizedAngleBetweenInputAngleAndStraightLineAngle(){
    ...
    Assertions.assertTrue(robotToGoal.getDegrees() > getAngleFromChassisSpeeds(magnetizedSpeed).getDegrees());
    Assertions.assertTrue(getAngleFromChassisSpeeds(magnetizedSpeed).getDegrees() > getAngleFromChassisSpeeds(robotSpeed).getDegrees());
  }
  ...
  void testMagnetismAngleGetsStrongerCloserToGoal(){
    ...
    ChassisSpeeds magnetizedSpeedClose = MagnetismUtil.getMagnetizedChassisSpeeds(robotSpeed, robotPoseClose, goalPose);
    ChassisSpeeds magnetizedSpeedFar = MagnetismUtil.getMagnetizedChassisSpeeds(robotSpeed, robotPoseFar, goalPose);


    Assertions.assertTrue(getAngleFromChassisSpeeds(magnetizedSpeedClose).getDegrees() > getAngleFromChassisSpeeds(magnetizedSpeedFar).getDegrees());
  }
