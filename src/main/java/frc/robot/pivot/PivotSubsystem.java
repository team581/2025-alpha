...
public class PivotSubsystem extends StateMachine<PivotState>{
    ...
    case STOWED -> {
          motor.setControl(
              motionMagicRequest.withPosition(
                  Units.degreesToRotations(PivotState.STOWED.angle)));
      }
    case CORAL_SCORE -> {
          motor.setControl(
              motionMagicRequest.withPosition(
                  Units.degreesToRotations(getScoreDirection().angle)));
      }
      default -> {}
    }
  }
  ...
  protected PivotState getNextState(PivotState currentState) {
      if (currentState == PivotState.HOMING && motorVelocity < RobotConfig.get().pivot().homingVelocityThreshold()) {
